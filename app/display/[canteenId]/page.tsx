...
interface Canteen {
  ...
  orders_display_interval?: number;
  orders_display_duration?: number;
}
...
export default function CanteenDisplayPage() {
  ...
  // Fetching data logic will be combined in fetchOrders to optimize and not repeat code
  ...
  const fetchOrders = async () => {
    ...
    try {
      const res = await fetch(`/api/display?canteenId=${canteenId}`);
      if (!res.ok) throw new Error('Failed to fetch from backend display route');
      const { canteen, orders } = await res.json();
      
      if (canteen) {
         setCanteen(canteen);
         setSlideshowEnabled(canteen.slideshow_enabled || false);
         setSlideshowInterval(canteen.slideshow_interval || 5000);
         setSlideshowItems(canteen.slideshow_items || []);
         setOrdersDisplayInterval(canteen.orders_display_interval || 10000);
         setOrdersDisplayDuration(canteen.orders_display_duration || 15000);
      }
